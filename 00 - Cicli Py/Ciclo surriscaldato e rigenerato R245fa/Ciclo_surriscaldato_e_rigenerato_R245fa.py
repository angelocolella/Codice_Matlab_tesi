...
h2 = CP.PropsSI('H', 'P', P2, 'S', s2, fluido)
...
#Parametri Surriscaldamento
Tsurr = 456
hsurr = CP.PropsSI('H', 'T', Tsurr, 'P', P2, fluido)
ssurr = CP.PropsSI('S', 'T', Tsurr, 'P', P2, fluido)


#Pt 4 (uscita turbina)
P4 = P1
s4 = 1959.440172083816
T4 = CP.PropsSI('T', 'P', P4, 'S', s4, fluido)
h4 = CP.PropsSI('H','P', P4, 'S', s4, fluido)

#Parametri rigenerazione
R = 0.75    #Efficienza rigenerazione 
T4r = T4 - R * (T4-T2)
h4r = CP.PropsSI('H', 'T', T4r, 'P', P4, fluido)
h2r = h2 + (h4 - h4r)
T2r = CP.PropsSI('T', 'P', P2, 'H', h2r, fluido)
...
#Trasformazione rigenerazione 2-2r

T22r_prov = linspace(T2, T2r, 20)
...
for i22r in range(len(T22r_prov)):
    s22r.append((CP.PropsSI('S', 'P', P3, 'T', T22r_prov[i22r], fluido))/1000)
    T22r.append((T22r_prov[i22r])-273.15)


#Trasformazione nella caldaia 2r-2e-3
...
T22e_prov = linspace(T2r, T2evs-0.1, 100)
...
#Trasformazione surriscaldamento 3-surr
T3surr_prov = linspace(T3+0.1, Tsurr, 5)
...
for i3surr in range(len(T3surr_prov)):
    s3surr.append((CP.PropsSI('S', 'P', P3, 'T', T3surr_prov[i3surr], fluido))/1000)
    T3surr.append((T3surr_prov[i3surr])-273.15)
...
Psurr4 = linspace(P3, P4, 500)


for i34 in range(len(Psurr4)):
    s4is = ssurr
    h4is = CP.PropsSI('H', 'P', Psurr4[i34], 'S', s4is, fluido)    

    #Sfrutto il rendimento di turbina eta_t=(h_3-h_4)/(h_3-h_4is) per determinare l'entalpia del punto 4 
    h4 = hsurr - eta_t * (hsurr-h4is)        
    ssurr4.append(CP.PropsSI('S', 'P', Psurr4[i34], 'H', h4, fluido)/1000)  
    Tsurr4.append((CP.PropsSI('T', 'P', Psurr4[i34], 'H', h4, fluido))-273.15)



#Rigenerazione 4-4r

T44r_prov = linspace(T4r, T4, 750)
...
for i44r in range(len(T44r_prov)):
    s44r.append((CP.PropsSI('S', 'P', P4, 'T', T44r_prov[i44r], fluido))/1000)
    T44r.append((T44r_prov[i44r])-273.15)


#Trasformazione nel condensatore 4r-1
#Tradormazione isobara P4 = P1
#4r-1

T4vs = CP.PropsSI('T', 'P', P4, 'Q', 1, fluido)
s4vs = CP.PropsSI('S', 'P', P4, 'Q', 1, fluido)
T41_prov = linspace(T4vs+0.1, T4r, 500)
...
for i41 in range(len(T41_prov)):
    ...
    if i41 == len(T41_prov):
        s41.append(s4vs/1000)
        T41.append(T4vs-273.15)


T4ls1 = [T4vs-273.15 ,T1-273.15]
s4ls1 = [s4vs/1000, s1/1000]
...
plot(s22r, T22r, color = colore)
plot(s22e, T22e, color = colore)
plot(s2e3, T2e3, color = colore)
plot(s3surr, T3surr, color = colore)
plot(ssurr4, Tsurr4, color = colore)
plot(s44r, T44r, color = colore)
plot(s41, T41, color = colore)
plot(s4ls1, T4ls1, color = colore)
title('Piano T-s ciclo surriscaldato e rigenerativo R245fa', fontdict=None, loc='center', pad=None)
